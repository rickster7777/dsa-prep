...
For each index:
1. Compute the index of the left child.
2. Compute the index of the right child.
3. If the left child exists and its value is less than the current node’s value, return false.
4. If the right child exists and its value is less than the current node’s value, return false.
5. If the loop finishes without any violations, return true./*
    Time Complexity: O(n)
