...
public class Main {
     ...
     * Uses a sliding window approach with HashMap (better for sparse data).
     * 
     * STRATEGY:
    ...
    private static int atMost(String s, int k) {
        ...
        // HashMap to store character -> frequency mapping
        // map.size() tells us how many distinct characters we have
        ...
        for (int right = 0; right < s.length(); right++) {
            ...
            // Add current character to map (or increment its frequency)
            ...
            while (map.size() > k) {
                ...
                // (reduces distinct character count)
